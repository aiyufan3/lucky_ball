...
def sample_without_replacement(
    weights: List[float], m: int, rng: random.Random
) -> List[int]:
...
def recommend_sets(
    entries: List[dict], sets: int = 5, rng_seed: int | None = None
) -> List[List[int]]:
...
def allocate_budget_by_score(
    ...
    while used > target and any(stakes):
        # 从最低分往下减
        i = (
            stakes.index(max(stakes))
            if used - target > 5
            else stakes.index(max(stakes))
        )
        stakes[i] -= 1
        if stakes[i] < 0:
            stakes[i] = 0
...
def write_profit_plan(
    ...
    if plan and any("ev_per_bet" in it for it in plan):
        lines.append(
            "> 免责声明：彩票随机，历史不代表未来；本方案基于 *玩法EV + 权重分散* 的示例方法，仅用于技术学习，不构成投注建议。\n"
        )
    else:
        lines.append(
    ...
    if planned_stakes == 0:
        lines.append(
            "- 注：当前 Kelly 严格模式在本玩法/赔率下给出 0 注（负EV或过低），可使用 --kelly_mode floor/ignore 查看非零分配示例。\n"
        )
    # 最近一期
    if entries:
        latest = entries[0]
        lines.append(
            f"- 最近开奖：期号 {latest.get('code')} | 日期 {latest.get('date')} | 号码：{', '.join(f'{x:02d}' for x in latest.get('nums', []))}\n"
        )
    ...
    if plan and any("ev_per_bet" in it for it in plan):
        ...
        if ev0 is not None and total_stakes > 0:
            ...
            lines.append(
                f"- 本期计划总注数：{total_stakes} 注；**理论期望盈亏**：{total_ev} 元\n"
            )
    lines.append("\n## 推荐组合与资金分配\n")
    total_amount = 0.0
    for item in plan:
...
def random_pick(
    k: int = 80, m: int = 20, rng: random.Random | None = None
) -> List[int]:
...
def backtest_overlap(
    ...
    for t in range(window, len(entries) - 1):
        ...
        random_ov.extend(
            monte_carlo_overlap(
                ...
                trials=max(
                    10,
                    random_trials // (len(entries) // max(1, (len(entries) - window))),
                ),
                k=80,
                m=20,
                rng_seed=rng.randint(1, 10**9),
            )
        )
    ...
    return {
        "model_overlaps": model_ov,
        "random_overlaps": random_ov,
        "summary": summary,
    }
...
def top_hot_and_cold(
    entries: List[dict], k: int = 80, topn: int = 10
...
def write_report(
    ...
    if meta:
        md.append(
            f"**和值均值(样本)**: {meta.get('sum_avg')} ；**理论期望**: {int(meta.get('sum_expected', 810))}\n"
        )
    md.append("\n## 热号 Top15\n")
...
def plot_frequency_hist(
    entries: List[dict], k: int = 80, out_path: str | None = None
) -> str:
    """号码频率直方图。"""
    freq = frequency_stats(entries, k=k)
    xs = list(range(1, k + 1))
...
def hypergeom_pmf_hits(
    n_choose: int, h: int, N_total: int = 80, draw: int = 20
) -> float:
...
def expected_value_for_pick(
    n_choose: int, payouts: dict, price_per_bet: float
) -> float:
...
def kelly_fraction_multi(
    n_choose: int, payouts: dict, price_per_bet: float, resolution: int = 500
) -> float:
    """
...
def recommend_pickn_sets(
    entries: List[dict], n: int, sets: int = 5, rng_seed: int | None = None
) -> List[List[int]]:
...
def allocate_budget_by_ev(
    ...
    if kelly_mode == "ignore":
        ...
        stakes_total = max(
            int(kelly_floor_stakes), int(round(kelly_f * bankroll_stakes))
        )
    ...
    if stakes_total <= 0:
        ...
        for idx, r in enumerate(recs, 1):
            sc = (
                ...
                else max(pick_weight_score(r, weights), 1e-12)
            )
            plan.append(
                {
                    "index": idx,
    ...
    # 用最大小数部分法补齐
    frac_order = sorted(
        range(len(recs)), key=lambda i: (ideal[i] - base[i]), reverse=True
    )
    for i in range(rem):
...
def main():
    ...
    parser.add_argument(
        "--limit", type=int, default=0, help="最近 N 期（0 表示接口默认分页）"
    )
    parser.add_argument(
        "--recommend", type=int, default=0, help="输出推荐组数（示例用途）"
    )
    ...
    parser.add_argument(
        "--backtest", action="store_true", help="执行滚动回测并与随机基线对比"
    )
    parser.add_argument("--window", type=int, default=200, help="回测窗口期数")
    parser.add_argument(
        "--sets_per_period", type=int, default=5, help="每期推荐的组合数（用于回测）"
    )
    parser.add_argument(
        "--random_trials", type=int, default=2000, help="随机基线样本量（总计近似）"
    )
    parser.add_argument(
        "--plots", action="store_true", help="生成频率直方图与EMA热力图"
    )
    parser.add_argument(
        "--plots_dual",
        action="store_true",
        help="生成上下分区红/蓝风格的频率分布图（视觉风格适配）",
    )
    parser.add_argument(
        "--split", type=int, default=40, help="双面板分割点（默认40，将1..split放上面）"
    )
    parser.add_argument(
        "--plan",
        action="store_true",
        help="根据权重分数与预算生成资金分配方案（学习用途）",
    )
    parser.add_argument("--budget", type=float, default=22.0, help="总预算金额（元）")
    parser.add_argument(
        "--price_per_bet", type=float, default=2.0, help="单注价格（元）"
    )
    parser.add_argument(
        "--play_pick",
        type=int,
        default=0,
        help="任选N（如 7 表示选七玩法）；>0 时按该玩法推荐与分配",
    )
    parser.add_argument(
        "--payouts",
        type=str,
        default=None,
        help="玩法赔率JSON路径（默认 data/payouts_kl8.json）",
    )
    parser.add_argument(
        "--beta", type=float, default=3.0, help="Softmax温度，越大分配越集中"
    )
    parser.add_argument(
        "--max_share", type=float, default=0.5, help="单组最大注数占比(0~1)"
    )
    parser.add_argument("--min_stake", type=int, default=1, help="EV>0时每组最小注数")
    parser.add_argument(
        "--kelly_mode",
        choices=["strict", "floor", "ignore"],
        default="strict",
    ...
    if args.backtest and history:
        bt = backtest_overlap(
            history,
            window=args.window,
            sets=args.sets_per_period,
            alpha=0.25,
            rng_seed=(args.seed or 123),
        ...
        if args.play_pick and args.play_pick > 0:
            ...
            if (
                ...
                plan_path = write_profit_plan(
                    history, recs if recs else [], plan, args.budget, price
                )
