...
class DoubleColorBallAnalyzer:
    ...
    def get_max_pages(self):
        ...
        for attempt in range(max_retries):
            try:
                ...
                data = response.json()
                print(
                    f"📊 API响应: state={data.get('state')}, message={data.get('message')}"
                )
    ...
    def fetch_lottery_data(self, max_pages=10):
        ...
        for page in range(1, max_pages + 1):
                        ...
                        try:
                            ...
                            for grade in prizegrades:
                                if grade.get("type") == 1:  # 一等奖
                                    first_prize_count = self._parse_number(
                                        grade.get("typenum", "0")
                                    )
                                    first_prize_amount = self._parse_number(
                                        grade.get("typemoney", "0")
                                    )
                                elif grade.get("type") == 2:  # 二等奖
                                    second_prize_count = self._parse_number(
                                        grade.get("typenum", "0")
                                    )
                                    second_prize_amount = self._parse_number(
                                        grade.get("typemoney", "0")
                                    )
            ...
            if consecutive_failures >= max_consecutive_failures:
                print(
                    f"🛑 连续 {max_consecutive_failures} 页失败，停止抓取以避免被封禁"
                )
                break

        print(
            f"🎉 数据抓取完成！成功抓取 {successful_pages} 页，共获取 {len(self.lottery_data)} 期开奖数据"
        )
    ...
    def compute_marginal_probs(
        self, decay_half_life=60, cond_weekday=None, shrink_beta=20.0
    ):
        """
        ...
        pr_g = (
            red_counts_g / red_counts_g.sum()
            if red_counts_g.sum() > 0
            else np.ones(33) / 33.0
        )
        pb_g = (
            blue_counts_g / blue_counts_g.sum()
            if blue_counts_g.sum() > 0
            else np.ones(16) / 16.0
        )
    ...
    def _marginal_probs_window(
        ...
        return tmp.compute_marginal_probs(
            decay_half_life=decay_half_life,
            cond_weekday=cond_weekday,
    ...
    def _three_way_fused_prior(self, decay_half_life=60):
        ...
        # 近窗 + 周几
        pr_short, pb_short = self._marginal_probs_window(
            decay_half_life=decay_half_life,
        ...
        # 全量 + 周几
        pr_wd, pb_wd = self.compute_marginal_probs(
            decay_half_life=decay_half_life,
            cond_weekday=target_wd,
            shrink_beta=SHRINK_BETA_WEEKDAY,
        )
        # 全量(不按周几)
        pr_g, pb_g = self.compute_marginal_probs(
            decay_half_life=decay_half_life,
            cond_weekday=None,
            shrink_beta=SHRINK_BETA_WEEKDAY,
        )
    ...
    def train_ml_model(
        self, seq_len=10, epochs=5, lr=1e-3, hidden_size=64, dropout=0.2
    ):
        """
        ...
        self.red_model = self._SeqPredictor(
            input_size, hidden_size, 33, output_type="sigmoid", dropout=dropout
        ).to(self.device)
        self.blue_model = self._SeqPredictor(
            input_size, hidden_size, 16, output_type="softmax", dropout=dropout
        ).to(self.device)
        ...
        self.red_model.train()
        self.blue_model.train()
        for ep in range(epochs):
            perm = torch.randperm(X_t.shape[0])
            X_t = X_t[perm]
            y_red_t = y_red_t[perm]
            y_blue_t = y_blue_t[perm]
            red_loss_sum = 0.0
            blue_loss_sum = 0.0
            for bi in range(num_batches):
                s = bi * batch_size
                e = min((bi + 1) * batch_size, X_t.shape[0])
                xb = X_t[s:e]
                yrb = y_red_t[s:e]
                ybb = y_blue_t[s:e]
                ...
                red_loss_sum += loss_red.item()
                blue_loss_sum += loss_blue.item()
            print(
                f"[LSTM] epoch {ep+1}/{epochs} red_loss={red_loss_sum/num_batches:.4f} blue_loss={blue_loss_sum/num_batches:.4f}"
            )
    ...
    def predict_next_probabilities(
        ...
        """
        # 三向融合先验（短窗/周几/全局）
        p_marg_red, p_marg_blue = self._three_way_fused_prior(
            decay_half_life=decay_half_life
        )
        if not self.trained:
        ...
        if blend_alpha == "auto":
            ...
            n_wd = sum(
                1
                for r in recent
                if self._weekday_from_date(r["date"]) == self._next_draw_weekday()
            )
    ...
    def _arima_sum_forecast(self, horizon=1):
        ...
        try:
            ...
            # predicted_mean can be ndarray/Series depending on input; coerce
            # to ndarray
    ...
    def _monte_carlo_candidates(
        ...
        for _ in range(n):
            ...
            if (
                sum_low is not None
                and sum_high is not None
                and not (sum_low <= s <= sum_high)
            ):
                continue
            # 选中集的熵与评分
            ...
            k_adapt = int(
                np.clip(6 - round(4 * sharp / (np.max(p_blue) + 1e-12)), 2, 6)
            )
            ...
            if merged:
                mask_hot = np.array(
                    [1 if i in hot_idx else 0 for i in merged], dtype=np.float64
                )
                w_hot = min(
                    0.40, float(mask_hot.sum()) / max(1.0, len(merged))
                )  # 上限 40%
                w_cold = 1.0 - w_hot
                if mask_hot.sum() > 0 and mask_hot.sum() < len(merged):
                    ...
                    sc = base_cold.sum()
                    if sh > 0:
                        base_hot = (base_hot / sh) * w_hot
                    if sc > 0:
                        base_cold = (base_cold / sc) * w_cold
    ...
    def analyze_patterns(self):
        ...
        for pattern, count in sorted(
            odd_even_dist.items(), key=lambda x: x[1], reverse=True
        ):
            percentage = (count / len(self.lottery_data)) * 100
            print(f"{pattern}: {count} 次 ({percentage:.1f}%)")

        print("\n和值分布统计：")
        for sum_range, count in sorted(
            sum_dist.items(), key=lambda x: int(x[0].split("-")[0])
        ):
            percentage = (count / len(self.lottery_data)) * 100
            print(f"{sum_range}: {count} 次 ({percentage:.1f}%)")

        print("\n跨度分布统计：")
        for span_range, count in sorted(
            span_dist.items(), key=lambda x: int(x[0].split("-")[0])
        ):
            percentage = (count / len(self.lottery_data)) * 100
    ...
    def generate_recommendations(self, num_sets=5):
        ...
        p_red, p_blue = self.predict_next_probabilities(
            blend_alpha="auto", decay_half_life=60
        )
        ...
        for i, rec in enumerate(recommendations, 1):
            ...
            print(
                f"推荐 {i}: {red_str} + {rec['blue_ball']:2d} | 熵:{rec['entropy_bits']:.3f}bits | 置信度:{rec['confidence']:.6f} | 和值:{rec['sum']} | 跨度:{rec['span']} | {rec['odd_even']}"
            )
    ...
    def evaluate_latest_draw(self, recommendations):
        ...
        if p_red is not None and p_blue is not None:
            ...
            blue_rank = int(
                (16 - np.argsort(np.argsort(p_blue))[blue_true - 1])
            )  # 1=最高
        ...
        print("\n=== 当期回测（最新期） ===")
        print(
            f"期号: {latest['period']} 日期: {latest['date']} 开奖: {sorted(list(reds_true))} + {blue_true:02d}"
        )
        print(
            f"推荐组合最佳红球重合数: {best_overlap} / 6 | 是否命中蓝球: {'是' if blue_hit else '否'}"
        )
        if p_red is not None:
            print(
                f"红球概率质量(真值总和): {red_mass:.4f} | 蓝球概率名次: Top-{blue_rank}{' (≤3)' if blue_top3 else ''}"
            )
        if blue_hit and best_overlap == 6:
            tier = "一等奖(理论)"
        elif best_overlap == 6:
            tier = "二等奖(理论)"
        elif best_overlap == 5 and blue_hit:
            tier = "三等奖(理论)"
        elif (best_overlap == 5) or (best_overlap == 4 and blue_hit):
            tier = "四等奖(理论)"
        elif (best_overlap == 4) or (best_overlap == 3 and blue_hit):
            tier = "五等奖(理论)"
        elif best_overlap == 2 and blue_hit:
            tier = "六等奖(理论)"
        else:
            tier = "未中奖(理论)"
    ...
    def generate_analysis_report(
        self, filename="reports/color_ball_analysis_report.md"
    ):
        """生成完整的分析报告文件"""
...
"""
        ...
        for i, (num, count) in enumerate(red_freq[:15], 1):
            percentage = (count / len(self.lottery_data)) * 100
            report_content += (
                f"| {i:02d} | **{num:02d}** | {count} | {percentage:.1f}% |\n"
            )
...
"""
        ...
        for i, rec in enumerate(recommendations, 1):
            ...
            report_content += (
                f"- 特征: {rec['odd_even']} | 和值:{rec['sum']} | 跨度:{rec['span']}\n"
            )
            ...
            report_content += (
                f"- 说明: {rec.get('description','LSTM+ARIMA+MonteCarlo 低熵组合')}\n\n"
            )
...
"""
    ...
    def _get_trends_analysis(self):
        """内部方法：获取趋势分析数据"""
        if len(self.lottery_data) < 10:
            return {"recent_draws": "数据不足", "hot_reds": "无", "hot_blues": "无"}
        ...
        hot_reds_str = (
            " ".join([f"{x:02d}" for x in sorted(hot_reds)]) if hot_reds else "无"
        )
        hot_blues_str = (
            " ".join([f"{x:02d}" for x in sorted(hot_blues)]) if hot_blues else "无"
        )
    ...
    def generate_aggregated_data_hjson(
        self, filename="data/lottery_aggregated_data.hjson"
    ):
        ...
        aggregated_data = {
            ...
            "metadata": {
                ...
                "latest_period": (
                    self.lottery_data[0]["period"] if self.lottery_data else None
                ),
                "latest_date": (
                    self.lottery_data[0]["date"] if self.lottery_data else None
                ),
            ...
            "frequency_analysis": {
                ...
                "red_balls": {
                    ...
                    "data": [
                        {
                            ...
                            "frequency": round(
                                (red_counter.get(num, 0) / len(self.lottery_data))
                                * 100,
                                2,
                            ),
                        }
                    ...
                    "summary": {
                        "highest_freq_number": (
                            max(red_counter.items(), key=lambda x: x[1])[0]
                            if red_counter
                            else None
                        ),
                        "highest_freq_count": (
                            max(red_counter.items(), key=lambda x: x[1])[1]
                            if red_counter
                            else 0
                        ),
                        "lowest_freq_number": (
                            min(red_counter.items(), key=lambda x: x[1])[0]
                            if red_counter
                            else None
                        ),
                        "lowest_freq_count": (
                            min(red_counter.items(), key=lambda x: x[1])[1]
                            if red_counter
                            else 0
                        ),
                        "average_frequency": round(
                            (
                                sum(red_counter.values()) / len(red_counter)
                                if red_counter
                                else 0
                            ),
                            2,
                ...
                "blue_balls": {
                    ...
                    "data": [
                        {
                            ...
                            "frequency": round(
                                (blue_counter.get(num, 0) / len(self.lottery_data))
                                * 100,
                                2,
                            ),
                        }
                    ...
                    "summary": {
                        "highest_freq_number": (
                            max(blue_counter.items(), key=lambda x: x[1])[0]
                            if blue_counter
                            else None
                        ),
                        "highest_freq_count": (
                            max(blue_counter.items(), key=lambda x: x[1])[1]
                            if blue_counter
                            else 0
                        ),
                        "lowest_freq_number": (
                            min(blue_counter.items(), key=lambda x: x[1])[0]
                            if blue_counter
                            else None
                        ),
                        "lowest_freq_count": (
                            min(blue_counter.items(), key=lambda x: x[1])[1]
                            if blue_counter
                            else 0
                        ),
                        "average_frequency": round(
                            (
                                sum(blue_counter.values()) / len(blue_counter)
                                if blue_counter
                                else 0
                            ),
                            2,
    ...
    def _get_trends_analysis_raw(self):
        """内部方法：获取原始趋势分析数据"""
        if len(self.lottery_data) < 10:
            return {"recent_draws": [], "hot_reds": [], "hot_blues": []}
    ...
    def update_readme_recommendations(
        ...
        try:
            ...
            current_time = (
                timestamp
                if timestamp
                else (datetime.now() + timedelta(hours=8)).strftime(
                    "%Y年%m月%d日 %H:%M:%S"
                )
            )
            ...
            for i, rec in enumerate(recommendations, 1):
                red_str = " ".join([f"{x:02d}" for x in rec["red_balls"]])
                rec_lines.append(
                    f"**推荐 {i}** ({rec.get('strategy','ML低熵')}): `{red_str}` + `{rec['blue_ball']:02d}`  "
                )
                rec_lines.append(
                    f"*{rec.get('description','LSTM+ARIMA+MonteCarlo 低熵组合')} | {rec['odd_even']} | 和值:{rec['sum']} | 跨度:{rec['span']}*"
                )
                rec_lines.append("")
...
def main():
    ...
    print("=" * 80)
    print(
        f"🧰 Runtime -> Python: {sys.version.split()[0]} | Torch: {getattr(torch, '__version__', 'N/A')} | CUDA: {torch.cuda.is_available() if hasattr(torch, 'cuda') else False}"
    )
    print(f"📄 Running script: {__file__}")
    if sys.version_info < (3, 11):
        print(
            "⚠️ 建议使用 Python 3.11+ 以避免 macOS LibreSSL/urllib3 警告，并获得更好的依赖兼容性。"
        )
    print("🎯 双色球数据分析系统")
    print("=" * 80)
    print("⚠️  重要免责声明：")
    ...
    try:
        analyzer.train_ml_model(
            seq_len=10, epochs=5, lr=1e-3, hidden_size=64, dropout=0.2
        )
    except Exception as e:
        print(f"⚠️  模型训练失败: {e}，将使用时间衰减频率作为备选。")
        analyzer.trained = False
    print(
        f"🧪 模型训练状态: {'已训练(使用LSTM融合)' if analyzer.trained else '未训练(使用时间衰减频率)'}"
    )
